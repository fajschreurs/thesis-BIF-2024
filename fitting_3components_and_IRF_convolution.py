...
data from FLIM (Fluorescence lifetime measurements) and fit a convolution
of the IRF (instrument response function).
...
def expdec_3(t, tau1, tau2, tau3, A, B, y0):
    ...
    """
    C = 1 - A - B  # Enforce A + B + C = 1, where C is the amplitude of tau3
...
def Convol(x, h):
    """
    convolutes two sets of data
    Parameters:
        x (array): The time values of dataset used for expdec3.
        h (array): other set of data (here IRF)
...
def jumpexpmodel_curve_3(X, tau1, tau2, tau3, ampl1, ampl2, y0):
    """
    Model with 3 components to be convoluted to IRF
    
    Parameters:
        X: array of timepoints
        tau1: array of lifetimes, first component
        tau2: array of lifetimes, second component
        tau3: array of lifetimes, third component
        ampl1: amplitude, gives attribution of first component to measured lifetime
        ampl2: amplitude, gives attribution of second component to measured lifetime
        y0: array, background of lifetimes
    Returns:
        z: convolved model of y model and IRF
    """
    x, irf = X
    ampl3 = 1 - ampl1 - ampl2
...
def mse_jumpexpmodel(p, X, ydata):
    """
    calculates mse of jumpexpmodel fit
    
    Parameters:
        p: parameters from jumpexpmodel fit
        X: data of x-axis, time array
        ydata: data of y-axis, intensity array of fluorescence
    Returns:
        mse(float): mean squared error of jumpexpmodel fit
    """
...
def load_irf(filepath):
    """
    loads IRF (instrument response function) file
    
    Parameters:
        filepath (dir): directory to file path
        
    Returns:
        irf_dict (dictionary): dictionary of irf data with decay and time
    """
...
def calc_adjusted_R_sq_jumpexp(data_to_fit, irf_data, popt_now):
    """
    calculates adjusted R squared of jumpexpmodel fit
...
def align_and_wrap_irf(time, irf_data, data):
    """
    alignes IRF peak in decay curve to peak in original fluorescence decay data
    
    Parameters:
        time: time array
        irf_data: IRF data of fluorescence decay
        data: original fluorescence decay data
        
    Returns:
        irf_data_wrapped: shifted IRF data
    """
...
if __name__ == "__main__":
    ...
    path = r"your input directory.sptw/"
    os.chdir(path)

    # Load the IRF data from the provided IRF file path
    irf_file = r"input directory IRF.ptu"
    irf_data = load_irf(irf_file)

    # List all PTU files in the directory and sort them by the z-value in the filename
    ptu_files = [f for f in os.listdir(path) if re.match(r'Series009_z\d+\.ptu', f)]#use regex fitting for .ptu file names
