...
and then saving in a CSV file the z-slice number and the corresponding ratio
between PSI and PSII intensities.
"""
...
def process_intensity_ratio(path, output_dir):
    """
    Processes PTU files in the given directory, calculates the average intensity
    for PSI and PSII, and saves the ratio to a CSV file.
    ...
    # Calculate the ratio between PSI and PSII
    ratio_psi_psii = average_intensities_psi / average_intensities_psii
    ...
    output_file_name = f"intensity_ratio_oefen_{folder_name}.csv"
    output_path = os.path.join(output_dir, output_file_name)

    # Write the data to the CSV file
    with open(output_path, mode='w', newline='') as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow(["z_number", "intensity_ratio"])  # Header
        writer.writerows(zip(z_values, ratio_psi_psii))  # Data rows
