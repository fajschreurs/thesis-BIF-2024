...
#Femke Schreurs aanpassingen in num_pixel x en y, daarna functie aangepast om te center croppen tot 500 pixels
        num_of_detectors     = np.unique(channel).size 
        num_tcspc_channel    = np.unique(tcspc).size
        num_pixel_X          = int(ImgHdr_PixX)
        num_pixel_Y          = int(ImgHdr_PixY)
        num_pixel_X_max      = 500
        num_pixel_Y_max      = 500

        flim_data_stack = np.zeros((int(num_pixel_Y), int(num_pixel_X), int(num_of_detectors), int(num_tcspc_channel)), dtype=np.uint16)
        # Apply center-cropping to 500x500 if the image dimensions are larger
        if num_pixel_Y > 500 and num_pixel_X > 500:
            start_x = int((num_pixel_X - 500) // 2)
            start_y = int((num_pixel_Y - 500) // 2)
            flim_data_stack = flim_data_stack[start_y:start_y + 500, start_x:start_x + 500, :, :]
            #print(start_x,start_y)
            #print(numba.typeof(flim_data_stack))
            #print(np.shape(flim_data_stack))
        else:
            raise ValueError("Image dimensions are smaller than 500x500 and cannot be cropped.")

        # Markers necessary to make FLIM image stack
        LineStartMarker = 2**(int(ImgHdr_LineStart-1))
        LineStopMarker  = 2**(int(ImgHdr_LineStop-1))
        FrameMarker     = 2**(int(ImgHdr_Frame-1))
        ...
        min_length = min(len(L2), len(L1))
        syncPulsesPerLine = np.floor(np.mean(L2[10:min_length] - L1[10:min_length]))
        # deze even vervangen door error z=11 syncPulsesPerLine = np.floor(np.mean(L2[10:] - L1[10:])) 
        ...
        for event in range(read_data_range+1):
            ...
            if not(isPhoton):         
                ...
                if (special_event == LineStartMarker):
                    ...
                    if (currentLine >= num_pixel_Y_max):
                        insideFrame  = False
                        currentLine  = 0
            ...
            if (isPhoton and insideLine and insideFrame):
                ...
                currentPixel = int(np.floor((((currentSync - syncStart)/syncPulsesPerLine)*num_pixel_X_max)))
                tmpchan   = channel[event]
                tmptcspc  = tcspc[event]
                sync = np.asarray(sync, dtype=np.uint64)
                tcspc = np.asarray(tcspc, dtype=np.uint16)
                channel = np.asarray(channel, dtype=np.uint8)
                special = np.asarray(special, dtype=np.uint8)
                #print(f"flim_data_stack shape: {flim_data_stack.shape}")
                #print(f"Accessing: currentLine={currentLine}, currentPixel={currentPixel}, tmpchan={tmpchan}, tmptcspc={tmptcspc}")
                #if (currentPixel < num_pixel_X_max) and (tmptcspc<num_tcspc_channel) and (currentLine < num_pixel_Y_max):
                #    flim_data_stack[currentLine][currentPixel][tmpchan][tmptcspc] += 1
                if (currentPixel < flim_data_stack.shape[1] and
                    currentLine < flim_data_stack.shape[0] and
                    tmpchan < flim_data_stack.shape[2] and
                    tmptcspc < flim_data_stack.shape[3]):
                    flim_data_stack[currentLine][currentPixel][tmpchan][tmptcspc] += 1
                    #print(type(flim_data_stack))
                else:
                    print(f"Out of bounds: Line={currentLine}, Pixel={currentPixel}, Channel={tmpchan}, TCSPC={tmptcspc}")
                    print(f"Stack shape: {flim_data_stack.shape}")
                #else:
                  #  print("Index out of bounds", currentLine, currentPixel, tmpchan, tmptcspc)
#         else:        
